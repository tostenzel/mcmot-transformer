"""
WILDTRACK sequence dataset from data in MOT format only for evaluation.

Only slightly adjusted copy of `mot17_sequence.py`

No detections in det, no test data without groundtruth/target
...
from wildtrack_globals import TRAIN_SEQ_LENGTH, TEST_SEQ_LENGTH
...
class WILDTRACKSequence(Dataset):
    ...
    """
    data_folder = 'WILDTRACK'
    # det is None for WILDTRACK by default
    def __init__(self, root_dir: str = 'data', seq_name: Optional[str] = None,
                 dets: str = None, vis_threshold: float = 0.0, img_transform: Namespace = None) -> None:
        """
        Args:
        ...
        self._train_folders = os.listdir(os.path.join(self._data_dir, 'mot-eval'))
        ...
        if seq_name is not None:
            ...
            if self._dets is not None:
                ...
                # ... or full_seq_name in self._train_folders or 
            assert full_seq_name in self._test_folders, \
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        data = self.data[idx]
        # read symlink
        img = Image.open(os.readlink(data['im_path']))#.convert("RGB")
    ...
    def _sequence(self) -> List[dict]:
        ...
        # Large changes for WILDTRACK img paths
        seq_name = self._seq_name.split('-')[0]
        eval_offset = 0
        # construct respective image names
        # set off by 5 in WILDTRACK and I use train, test, and val
        # splits in this order.
        if self._seq_name.split('-')[1] == "test":
            eval_offset = TRAIN_SEQ_LENGTH * 5
        elif self._seq_name.split('-')[1] == "val":
            eval_offset = (TRAIN_SEQ_LENGTH + TEST_SEQ_LENGTH) * 5
        elif self._seq_name.split('-')[1] == "train" or "eval_train":
            eval_offset = 0
        total = [
            {
            'gt': boxes[i],
            'im_path': osp.join(
                img_dir, seq_name + "-" + f"{((i) * 5 + eval_offset):08d}.jpg"),
            'vis': visibility[i],
            'dets': dets[i]
            }
    ...
    def get_track_boxes_and_visbility(self) -> Tuple[dict, dict]:
        ...
        visibility = {}
        # not 1 to seq len + 1 as for MIT       
        for i in range(0, self.seq_length + 0):
            boxes[i] = {}
            visibility[i] = {}
    ...
    def get_seq_path(self) -> str:
        ...
        if full_seq_name in self._test_folders:
            return osp.join(self._data_dir, 'mot-eval', full_seq_name)
        #else:
    ...
    def get_gt_file_path(self) -> str:
        ...
        return osp.join(self.get_seq_path(), 'gt', f'{self._seq_name}_gt.txt')
    ...
    # WILDTRACK without dets appendum
